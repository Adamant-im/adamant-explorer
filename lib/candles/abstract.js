...
function AbstractCandles (client) {
    ...
    this.retrieveTrades = function (start, cb) {
        ...
        console.log('Retrieving trades from', self.name + '...');
        ...
        async.doUntil(
            function (next) {
                request.get({
                    ...
                    var message = body[self.response.error],
                        data    = self.rejectTrades(body[self.response.data] || body);

                    if (err || resp.statusCode != 200) {
                        return next(err || 'Response was unsuccessful');
                    } else if (message) {
                        return next(message);
                    } else if (self.validTrades(results, data)) {
                        console.log(start.toString(), '=> Found', data.length.toString(), 'trades');
            ...
            function (err) {
                if (err) {
                    ...
                    console.log(results.length.toString(), 'trades in total retrieved');
    ...
    this.parseDate = function (date) {
        if (typeof date == 'string') {
            return moment(date).startOf(self.duration);
        } else {
            return moment.unix(date).startOf(self.duration);
        }
    }
    ...
    this.sumTrades = function (results, cb) {
        var sum = _.map(results, function (period) {
            return {
                ...
                xcrVolume  : _.reduce(period, function (memo, t) { return (memo + parseFloat(t[self.candle.amount])) }, 0.0).toFixed(8),
        ...
        sum = _.reject(sum, function (s) {
            return self.parseDate(s.timestamp).unix() === moment().startOf(self.duration).unix();
        });
    ...
    this.saveCandles = function (results, cb) {
        ...
        multi.exec(function (err, replies) {
            if (err) {
                ...
                console.log(replies.length.toString(), 'candles saved');
                return cb(null, results);
            }
        });
    }
    ...
    this.restoreCandles = function (duration, index, cb) {
        client.LRANGE(self.candleKey(duration), index, -1, function (err, reply) {
            if (err) {
                ...
                console.log(reply.length.toString(), 'candles restored');
                return cb(null, reply);
            }
        });
    }

    this.rebuildCandles = function (cb) {
        async.eachSeries(self.durations, function (duration, callback) {
            self.duration = duration;
            _rebuildCandles(callback);
    ...
    var _rebuildCandles = function (cb) {
        console.log('rebuilding', self.duration, 'candles for', self.name + '...');
    ...
    var _updateCandles = function (cb) {
        console.log('Updating', self.duration, 'candles for', self.name + '...');
