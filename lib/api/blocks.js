...
module.exports = function (app) {
  this.getBlockHeight = function (error, success) {
    return axios({
      url: app.get('adamant address') + '/api/blocks/getHeight',
      method: 'get',
    })
      .then((response) => {
  ...
  this.getBlockByHeight = function (height, error, success) {
    return axios({
      url: app.get('adamant address') + '/api/blocks?height=' + height,
      method: 'get',
    })
      .then((response) => {
        if (response.status !== 200) {
  ...
  function Block() {
    this.getBlock = function (blockId, height, cb) {
      return axios({
        url: app.get('adamant address') + '/api/blocks/get?id=' + blockId,
        method: 'get',
      })
        .then((response) => {
          if (response.status !== 200) {
    ...
    const makeBody = function (body, height) {
      ...
      b.payloadHash = Buffer.from(b.payloadHash).toString('hex');
  ...
  this.getBlockStatus = function (error, success) {
    return axios({
      url: app.get('adamant address') + '/api/blocks/getStatus',
      method: 'get',
    })
      .then((response) => {
        if (response.status !== 200) {
