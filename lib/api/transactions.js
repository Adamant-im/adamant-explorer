...
module.exports = function (app) {
    ...
    this.getLastTransactions = function (error, success) {
        request.get({
            url : app.get('crypti address') + '/api/transactions?orderBy=t_timestamp:desc&limit=20',
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
                return error({ success : false, error : (err || 'Response was unsuccessful') });
    ...
    this.getTransactionsByAddress = function (query, error, success) {
        ...
        request.get({
            url : app.get('crypti address') + '/api/transactions?recipientId=' + query.address + '&senderId=' + query.address + '&orderBy=t_timestamp:desc' + '&offset=' + param(query.offset, 0) + '&limit=' + param(query.limit, 100),
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
                return error({ success : false, error : (err || 'Response was unsuccessful') });
    ...
    this.getTransactionsByBlock = function (query, error, success) {
        ...
        request.get({
            url : app.get('crypti address') + '/api/transactions?blockId=' + query.blockId + '&orderBy=t_timestamp:desc' + '&offset=' + param(query.offset, 0) + '&limit=' + param(query.limit, 100),
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
                return error({ success : false, error : (err || 'Response was unsuccessful') });
