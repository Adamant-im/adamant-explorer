...
module.exports = function (app) {
    ...
    this.getTransactionsByAddress = function (query, error, success) {
        ...
        request.get({
            url : app.get('lisk address') + '/api/transactions?'+(query.direction!=='1' ? 'recipientId=' + query.address + '&' : '')+(query.direction!=='2' ? 'senderId=' + query.address + '&' : '')+'orderBy=timestamp:desc' + '&offset=' + param(query.offset, 0) + '&limit=' + param(query.limit, 100),
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
                return error({ success : false, error : (err || 'Response was unsuccessful') });
