...
module.exports = function (app) {
      ...
      function (waterCb) {
        request.get({
          ...
          if (err || response.statusCode !== 200) {
            return error({
              success: false,
              error: (err || 'Response was unsuccessful')
            })
  ...
  this.getLastTransfers = function (error, success) {
    request.get({
      url: app.get('lisk address') + '/api/transactions?orderBy=timestamp:desc&limit=20&minAmount=1',
      json: true
    }, function (err, response, body) {
      if (err || response.statusCode !== 200) {
        return error({
      ...
      function (cb) {
        ...
        data.forEach(function (directionQuery, index) {
          ...
          if (isTransfersOnly && ~directionQuery.indexOf('senderId') && ~directionQuery.indexOf('recipientId')) {
            ...
            uri = '/api/transactions?minAmount=1&' + directionQuery.split('recipientId')[0] + 'inId=' + address;
          } else if (isTransfersOnly) {
            uri = '/api/transactions?minAmount=1&' + directionQuery;
          }

          request.get({
            url: app.get('lisk address') + uri,
            json: true
          }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
