...
module.exports = function (app) {
    ...
    function Locations() {
        ...
        this.url = function (ip) {
            return '/json/' + ip;
        }

        this.collect = function (p, body) {
            body.osBrand = p.osBrand;
            this.list.push(body);
        }
    }

    this.getLocations = function (peers, error, success) {
        ...
        async.doUntil(
            function (next) {
                ...
                request.get({
                    url: app.get("freegeoip address") + locations.url(p.dottedQuad),
                    json : true
                }, function (err, resp, body) {
                    if (err || resp.statusCode != 200) {
                        return next(err || "Status code is not equal 200");
                    } else {
                        locations.collect(p, body);
                        return next();
                    }
                });
            },
            ...
            function (err) {
                if (err) {
                    ...
                    return success({ success: true, locations: locations.list });
                }
            }
        );
    }
