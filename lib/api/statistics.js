...
module.exports = function (app) {
    function Blocks () {
        this.maxOffset = 1000; // 1100
        this.maxCount  = 1080; // 3 hours
    ...
    function Peers (locator) {
        ...
        this.unique = function () {
            this.list.connected    = _.chain(this.list.connected).indexBy('ip').values().value();
            this.list.disconnected = _.chain(this.list.disconnected).indexBy('ip').values().value();
            return this.list;
        }
        ...
        async.doUntil(
            ...
            function (err) {
                ...
                if (err) {
                    ...
                    return success({ success : true, list : peers.unique() });
                }
            }
        );
    };
