    ...
    function Locator () {
        ...
        this.locate = function (ip, cb) {
            ...
            if (this.disabled) {
                ...
                // logger.info('Locator:', 'Using cached location for:', ip);
                return cb(location);
            } else {
                logger.info('Locator:', 'Requesting new location for:', ip);
        ...
        this.update = function (ips) {
            for (var ip in cache) {
                if (ips.indexOf(ip) === -1) {
                    logger.info('Locator', 'Removing stale location:', ip);
                    delete cache[ip];
                }
            }
        };
        ...
        var getLocation = function (ip, cb) {
            ...
            function (err, res) {
                if (err) {
                    logger.error('Locator:', 'Error retrieving location:', err);
                }
        ...
        var getFreegeoip = function (ip, cb) {
            request.get({
                ...
                if (err || resp.statusCode !== 200) {
                    logger.error('Locator:', 'Failed to get new location for:', ip);
                    return cb(err);
                } else {
                    return cb(null, body);
                }
        ...
        var getHostName = function (ip, cb) {
            dns.reverse(ip, function (err, hostnames) {
                ...
                if (err) {
                    logger.error('Locator:', 'Failed to get new hostname for:', ip);
                    hostname = ip + '.unknown';
                } else {
    ...
    function Peers (locator) {
        ...
        this.collect = function (peers) {
            ...
            async.doUntil(
                ...
                function (err) {
                    if (err) {
                        logger.info('Error collecting peers: ' + err);
                        return [];
                    } else {
                        return peers;
                    }
    ...
    this.getPeers = function (error, success) {
        ...
        async.doUntil(
            ...
            function (err) {
                ...
                if (err) {
                    logger.info('Error retrieving Peers: ' + err);
                    return error({ success : false, error : err });
                } else {
