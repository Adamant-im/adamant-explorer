...
module.exports = function (app, api) {
    ...
    this.getFee = function (error, success) {
        request.get({
            url : app.get('lisk address') + '/api/blocks/getFee',
            json : true
    ...
    this.getLISKCourse = function (error, success) {
        if (exchange.LISKBTC) {
            if (exchange.LISKBTC == '~') {
                return success({ success : true, lisk : (0).toFixed(8), usd : (0).toFixed(2) });
            } else {
                return success({ success : true, lisk : exchange.LISKBTC, usd : exchange.convertLISKTOUSD(1) });
            }
        } else {
