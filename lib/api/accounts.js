...
module.exports = function (app) {
    function Account () {
        this.validAddress = function (address) {
            return (
                typeof address === 'string' && address.match(/[0-9]{16,20}C/)
            );
        };
        ...
        this.getDelegate = function (account, cb) {
            ...
            request.get({
                url : app.get('crypti address') + '/api/delegates/get?publicKey=' + account.publicKey,
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
        ...
        this.getForged = function (account, cb) {
            ...
            request.get({
                url : app.get('crypti address') + '/api/delegates/forging/getForgedByAccount?generatorPublicKey=' + account.publicKey,
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
    ...
    this.getTopAccounts = function (query, error, success) {
        ...
        request.get({
            url : app.get('crypti address') + '/api/accounts/top?&offset=' + param(query.offset, 0) + '&limit=' + param(query.limit, 100),
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
