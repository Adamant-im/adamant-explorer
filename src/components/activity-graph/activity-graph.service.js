...
ActivityGraph.prototype.addTx = function (tx) {
  if (this.indexes.includes(tx.id)) {
    return;
  }
  this.addNode({
    id: tx.id,
...
ActivityGraph.prototype.amount = (tx, sign) => `${sign + tx.amount / Math.pow(10, 8)} ADM`;
...
ActivityGraph.prototype.addBlock = function (block) {
  if (this.indexes.includes(block.id)) {
    return;
  }
  if (this.blocks + 1 > this.maxBlocks) {
    this.clear();
  }
  this.addNode({
    id: block.id,
...
AppActivityGraph.factory('activityGraph', ($socket, $rootScope) => (vm) => {
  ...
  ns.on('data', (res) => {
    activityGraph.refresh(res.block);
  });
