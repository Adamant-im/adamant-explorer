...
const NetworkMap = function () {
    ...
    for (let item of peers.connected) {
      ...
      if (!Object.keys(this.markers).includes(item.ip)) {
        this.cluster.addLayer(
          (this.markers[item.ip] = L.marker([item.location.latitude, item.location.longitude], {
            title: item.ipString,
            icon: platformIcons[item.osBrand.name],
  ...
  var validLocation = (location) =>
    location && angular.isNumber(location.latitude) && angular.isNumber(location.longitude);

  var popupContent = (p) => {
    let content = '<p class="ip">'.concat(p.ip, '</p>');

    if (p.location.hostname) {
      content += '<p class="hostname">'.concat(
    ...
    content += '<p class="version">'.concat(
      '<span class="label">Version: </span>',
      p.version,
      '</p>',
    );

    content += '<p class="os">'.concat('<span class="label">OS: </span>', p.os, '</p>');

    if (p.location.city) {
      content += '<p class="city">'.concat(
    ...
    if (p.location.region_name) {
      content += '<p class="region">'.concat(
        '<span class="label">Region: </span>',
        p.location.region_name,
        '</p>',
      );
    }

    if (p.location.country_name) {
      content += '<p class="country">'.concat(
...
const NetworkMonitor = function (vm) {
  ...
  function Versions(peers) {
    const inspect = () => {
      if (angular.isArray(peers)) {
        return uniq(peers.map((p) => p.version).sort())
          .reverse()
          .slice(0, 3);
      } else {
        return [];
  ...
  function Heights(peers) {
    const inspect = () => {
      if (angular.isArray(peers)) {
        return uniq(peers.map((p) => p.height).sort((a, b) => a - b))
          .reverse()
          .slice(0, 4);
      } else {
        return [];
...
AppNetworkMonitor.factory('networkMonitor', ($socket, $rootScope) => (vm) => {
