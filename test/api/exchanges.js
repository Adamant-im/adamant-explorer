...
describe.skip('Exchanges API (Market Watcher)', function () {
  ...
  function getExchanges(done) {
    testUtils.httpRequest.get('/api/exchanges', done);
  }

  function getCandles(e, d, done) {
    testUtils.httpRequest.get('/api/exchanges/getCandles?e=' + e + '&d=' + d, done);
  }

  function getStatistics(e, done) {
    testUtils.httpRequest.get('/api/exchanges/getStatistics?e=' + e, done);
  }
  ...
  function getOrders(id, done) {
    testUtils.get('/api/exchanges/getOrders?e=' + id, done);
  }

  function checkOrders(id) {
    for (let i = 0; i < id.length; i++) {
      if (id[i + 1]) {
        testUtils.expect(id[i]).to.have.all.keys('ask', 'bid', 'price', 'amount');
      }
    }
  }
  ...
  function checkValues(id) {
    for (let i = 0; i < id.length; i++) {
      if (id[i + 1]) {
        testUtils.expect(id[i][0]).to.be.a.number;
        testUtils.expect(id[i][1]).to.be.a.number;
      }
    }
  }
  ...
  describe('GET /api/exchanges', function () {
    it('should be ok', function (done) {
      getExchanges((err, res) => {
        ...
        testUtils.expect(res.body).to.have.property('enabled').to.be.ok;
        testUtils
          .expect(res.body)
  ...
  describe('GET /api/exchanges/getCandles', function () {
    ...
    it('using hour for bittrex should be ok and return timeframe hour', function (done) {
      getCandles('bittrex', 'hour', (err, res) => {
        testUtils.expect(res.body).to.have.property('success').to.be.ok;
        testUtils.expect(res.body).to.have.property('timeframe').to.be.equal('hour');
        testUtils
          .expect(res.body)
          .to.have.property('exchange')
          .to.be.equal('bittrex');
    ...
    it('using day for bittrex should be ok and return timeframe day', function (done) {
      getCandles('bittrex', 'day', (err, res) => {
        testUtils.expect(res.body).to.have.property('success').to.be.ok;
        testUtils.expect(res.body).to.have.property('timeframe').to.be.equal('day');
        testUtils
          .expect(res.body)
          .to.have.property('exchange')
          .to.be.equal('bittrex');
  ...
  describe('GET /api/exchanges/getOrders', function () {
    it('using bittrex should be ok', function (done) {
      getOrders('bittrex', (err, res) => {
        testUtils.expect(res.body).to.have.property('success').to.be.ok;
        testUtils.expect(res.body).to.have.property('orders');
