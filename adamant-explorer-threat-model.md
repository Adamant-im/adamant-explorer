...
| --- | --- | --- | --- | --- |
| Retained HTTP API | Same-origin or direct GET/HEAD | Internet → Express | 12 UI routes plus network health | `api/lib/adamant/helpers/http.js` / `SUPPORTED_API_PATHS` |
...
| TM-003 | Malicious/MITM Node | Operator selects an untrusted or plaintext remote node | Return false chain/network data that is cached and displayed | Misleading balances, blocks, delegate/health state | Data integrity | HTTPS public defaults; SDK health/failover (`api/lib/adamant/requests/api.js`); block-versioned cache keys (`cache.js`) | Operators may still configure plaintext remote nodes; no consensus cross-check | Prefer HTTPS/private links and validate response schemas in #35 | Compare heights/hashes across independently operated nodes | Low | High | Medium |
...
| `api/lib/adamant/helpers/http.js` | Exact supported API contract prevents legacy route revival | TM-001, TM-006 |
